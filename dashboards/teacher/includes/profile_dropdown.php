<?php
/**
 * Teacher Profile Dropdown Component
 * Google-style profile dropdown with notifications
 */

// Get current teacher data if not already fetched
if (!isset($teacher)) {
    $db_header = new Database();
    $conn_header = $db_header->getConnection();
    
    $query_header = "SELECT t.*, u.email, u.username, u.profile_picture, u.status 
                     FROM teachers t 
                     JOIN users u ON t.user_id = u.user_id 
                     WHERE t.teacher_id = :teacher_id";
    $stmt_header = $conn_header->prepare($query_header);
    $stmt_header->bindParam(':teacher_id', $_SESSION['teacher_id']);
    $stmt_header->execute();
    $teacher = $stmt_header->fetch(PDO::FETCH_ASSOC);
}
    ...
    <div class="profile-dropdown" id="profileDropdown">
        <div class="profile-dropdown-header">
            ...
            <div class="profile-dropdown-info">
                <div class="profile-dropdown-name"><?php echo htmlspecialchars($teacher['first_name'] . ' ' . $teacher['last_name']); ?></div>
        ...
        <div class="profile-dropdown-menu">
            ...
            <a href="students.php" class="profile-dropdown-item">
